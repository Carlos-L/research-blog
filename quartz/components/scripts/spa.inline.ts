...
const normalizePathname = (pathname: string) => {
  ...
  const doubledBasePath = `${basePath}${basePath}`
  while (normalized.startsWith(doubledBasePath)) {
...
const normalizeInternalUrl = (url: URL) => {
...
const getOpts = ({ target }: Event): { url: URL; scroll?: boolean } | undefined => {
  ...
  return {
    url: normalizeInternalUrl(new URL(href)),
